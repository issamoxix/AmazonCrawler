...
def main():
    ...
    for key, item in enumerate(items):
        ...
        threading.Thread(save_image(extracting_name, title, image_url, key))
        ...
        if product_checker[0] == 0 and len(product_checker) == 1:
            threading.Thread(target=get_item_details(key, item_url, ITEMS_DICT)).start()
            # get_item_details(key, item_url, ITEMS_DICT)
